...
with tab_mom:
    render_momentum_tab(f1r, f1c, cfg["name"], unit, key_prefix=f"energy_{product_code}", phase=phase)

with tab_carry:
    render_carry_tab(curve, f1r, f1c, cfg["name"], unit, key_prefix=f"energy_{product_code}", phase=phase)

with tab_val:
    contracts = [c for c in curve.columns if c.startswith("F") and c[1:].isdigit() and int(c[1:]) <= 15]
    render_value_tab(curve, f1r, f1c, cfg["name"], unit, key_prefix=f"energy_{product_code}",
