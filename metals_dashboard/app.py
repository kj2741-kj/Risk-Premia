...
import metals as metals_research_cfg
...
key_prefix = f"metals_{cfg['code']}"

with tab_mom:
    mom_positions = render_momentum_tab(f1r, f1c, metal, cfg["unit"], key_prefix=key_prefix, phase=phase)

with tab_carry:
    carry_positions = render_carry_tab(curve, f1r, f1c, metal, cfg["unit"], key_prefix=key_prefix, phase=phase)

with tab_val:
    contracts = [c for c in curve.columns if c.startswith("F") and int(c[1:]) <= 15]
    value_positions = render_value_tab(curve, f1r, f1c, metal, cfg["unit"], key_prefix=key_prefix,
                                        contracts=contracts, phase=phase)

with tab_compare:
    render_comparison_tab(f1r, f1c, metal, cfg["unit"], key_prefix=key_prefix, phase=phase,
